...
const { BARANGAYS } = require("../utils/barangays");
...
1. "intent": Exactly one of:
   - "EMERGENCY_REPORT" (Active disaster, casualty, fire, rising flood, accident, trapped persons)
   - "RESOURCE_REQUEST" (Asking for food, water, rescue boat, transport to evacuation center)
   - "STATUS_INQUIRY" (Asking for water levels, rescue arrival ETA, updates)
   - "CASUALTY_REPORT" (Reporting injuries, missing persons, deceased)
   - "CASUAL_OR_GREETING" (Greeting, test message, bot chit-chat)
   - "FEEDBACK_OR_THANKS" (Thanking responders, general appreciation)
   - "OTHER" (Spam, unclear, or out-of-scope)

2. "urgency": Exactly one of:
   - "CRITICAL" (Immediate danger to life: drowning, active fire, trapped, severe injury, giving birth)
   - "HIGH" (Rising flood entering home, missing family member, power lines down in water)
   - "MEDIUM" (Needs food/clean water, stranded but safe, relief supply request)
   - "LOW" (Inquiries, greetings, gratitude, status checks)

3. "incident_type": Exactly one of:
   ["flood", "fire", "landslide", "accident", "medical", "evacuation", "casualty", "earthquake", "typhoon", "none"]
...
async function main() {
    ...
    const existingMap = loadExistingAnnotations();
    console.log(`📁 Total records in input: ${allRecords.length}`);
    console.log(`🔄 Already annotated:     ${existingMap.size}`);

    const pending = allRecords.filter(r => !existingMap.has(r.id));
    ...
    const appendStream = fs.createWriteStream(OUTPUT_JSONL, { flags: "a", encoding: "utf8" });
        ...
        try {
            ...
            for (const item of batch) {
                ...
                const annotatedRecord = {
                    ...
                    incident_type: aiResult.incident_type || item.incident_type || "none",
